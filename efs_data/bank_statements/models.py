...
class APICallRecord(models.Model):
    reference = models.CharField(max_length=255, null=True, blank=True)  # API data
    submission_time = models.DateTimeField(null=True, blank=True)  # API data
    abn = models.CharField(max_length=20, null=True, blank=True)  # ABN coming from API
    acn = models.CharField(max_length=20, null=True, blank=True)
    bank = models.ForeignKey(Bank, on_delete=models.CASCADE, related_name='api_calls', to_field='bank_id')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"API Call {self.reference} at {self.submission_time}"
